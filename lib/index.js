...
function rule(val, data, options) {
  ...
  var valueForKey = (0, _get3["default"])(data, (0, _utils.getPath)(path, prefix), undefined);
...
function checkRule(block, data, results, options) {
  if (Object.prototype.hasOwnProperty.call(block, 'and')) return block.and.every(function (item) {
    return checkRule(item, data, results, options);
  });
  if (Object.prototype.hasOwnProperty.call(block, 'or')) return block.or.some(function (item) {
    return checkRule(item, data, results, options);
  });
